...
class ResNet(nn.Module):
    ...
    """
    def __init__(self, block, num_blocks, num_classes=10, device='cpu', initial_train=0):
        ...
        self.drop_out = nn.Dropout(p=0.9)
        if False:
    ...
    def forward(self, x):
        ...
        # feature = F.dropout(feature, p=0.9, training=self.get_score)
        out = self.linear(out)
        return out, feature
...
def resnet(name, num_classes=10, pretrained=False, device='cpu', initial_train=0):
    """
    ...
    """
    if name == 'resnet18':
        return ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, device=device, initial_train=initial_train)
    elif name == 'resnet34':
        return ResNet(BasicBlock, [3, 4, 6, 3], num_classes=num_classes, device=device, initial_train=initial_train)
    elif name == 'resnet50':
